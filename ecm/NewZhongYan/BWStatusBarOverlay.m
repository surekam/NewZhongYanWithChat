...
- (void)showMessage:(NSString *)message duration:(NSTimeInterval)duration animated:(BOOL)animated {
    if (YES == self.hidden) {
...
+(void)showMessage:(NSString *)message duration:(NSTimeInterval)duration animated:(BOOL)animated
{
    [[BWStatusBarOverlay shared] showMessage:message duration:duration animated:animated];
}
...
- (void)setStatusBarStyle:(UIStatusBarStyle)statusBarStyle {
    if (UIStatusBarStyleBlackOpaque == statusBarStyle ||
        ...
        if (IS_IOS7) {
            [self setBackgroundColor:COLOR(0, 97, 194)];
            self.textLabel.textColor = [UIColor whiteColor];
            [self setProgressBackgroundColor:[UIColor colorWithRed:48/255.0f green:159/255.0f blue:211/255.0f alpha:1]];
